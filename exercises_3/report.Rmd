...
```{r green_lm_plot1, echo=FALSE, fig.width = 5, fig.height = 3, fig.align='center', out.width='.49\\linewidth', fig.show='hold', comment=NA, warning=FALSE}
train_cases = sample.int(n, n_train, replace=FALSE)
test_cases = setdiff(1:n, train_cases)
on_train = green[train_cases, 2:ncol(green)]
on_test = green[test_cases, 2:ncol(green)]
...
ggplot() +
  ...
  xlab("Actual Rent Price") + 
  ylab("Predicted Rent Price") + 
  ggtitle("Actual Rent vs. Predicted Rent for LM1")
...
ggplot() +
  ...
  xlab("Actual Rent Price") + 
  ylab("Predicted Rent Price") + 
  ggtitle("Actual Rent vs. Predicted Rent for LM2")
```

```{r green_lm_rmse, echo=FALSE, comment=NA}
cat("LM1 = LINEAR REGRESSION MODEL (without interactions) - RMSE:", lm_avg[1])
cat("LM2 = LINEAR REGRESSION MODEL (with interactions) - RMSE:", lm_avg[2])
```
...
for (k_val in k_vals) {
  rmse_vals_iter = do(num_splits)*{
    ...
    #create KNN model
    train_temp = model.matrix.lm(Rent ~ . - 1, data = on_train, na.action = na.pass)
    test_temp = model.matrix.lm(Rent ~ . - 1, data = on_test, na.action = na.pass)
    Xtrain_temp = train_temp[complete.cases(train_temp),]
    Xtest_temp = test_temp[complete.cases(test_temp),]
    
    ytrain = on_train$Rent[complete.cases(train_temp)]
    ytest = on_test$Rent[complete.cases(test_temp)]
...
```{r green_knn_rmse_plot, echo=FALSE, fig.width = 5, fig.height = 3, fig.align='center', out.width='.49\\linewidth', fig.show='hold', comment=NA, warning=FALSE}
...
```{r green_knn_plot1, echo=FALSE, fig.width = 5, fig.height = 3, fig.align='center', out.width='.49\\linewidth', fig.show='hold', comment=NA, warning=FALSE}
# re-split into train and test cases with the same sample sizes
train_cases = sample.int(n, n_train, replace=FALSE)
test_cases = setdiff(1:n, train_cases)
on_train = green[train_cases,2:ncol(green)]
...
ggplot() +
  ...
  xlab("Actual Rent Price") + 
  ylab("Predicted Rent Price") + 
  ggtitle("Actual Rent vs. Predicted Rent for KNN Model")
```
...
```{r green_lasso_ridge_plots, echo=FALSE, fig.width = 5, fig.height = 3, fig.align='center', out.width='.49\\linewidth', fig.show='hold', comment=NA, warning=FALSE}
# re-split into train and test cases with the same sample sizes
train_cases = sample.int(n, n_train, replace=FALSE)
test_cases = setdiff(1:n, train_cases)
on_train = green[train_cases,2:ncol(green)]
...
ggplot() +
  geom_point(data=data.frame(x = y_test, y = as.numeric(unname(y_pred_l)) ),aes(x = x, y = y ), color="black") + 
  geom_line(data=data.frame(x = y_test, y = y_test),aes(x = x, y = y), color="red") + 
  xlab("Actual Rent Price") + 
  ylab("Predicted Rent Price") + 
  ggtitle("Actual Rent vs. Predicted Rent for LR")

ggplot() +
  geom_point(data=data.frame(x = y_test, y =as.numeric(unname(y_pred_r)) ),aes(x = x, y = y ), color="black") + 
  geom_line(data=data.frame(x = y_test, y = y_test),aes(x = x, y = y), color="red") + 
  xlab("Actual Rent Price") + 
  ylab("Predicted Rent Price") + 
  ggtitle("Actual Rent vs. Predicted Rent for RR")
```

```{r green_lasso_ridge_rmse, echo=FALSE, comment=NA}
cat("LR = LASSO REGRESSION - RMSE:", lr_model_avg[1])
cat("RR = RIDGE REGRESSION - RMSE:", rr_model_avg[1])
```
...
```{r green_logit_plot1, echo=FALSE, fig.width = 5, fig.height = 3, fig.align='center', out.width='.49\\linewidth', fig.show='hold', comment=NA, warning=FALSE}
# re-split into train and test cases with the same sample sizes
train_cases = sample.int(n, n_train, replace=FALSE)
test_cases = setdiff(1:n, train_cases)
on_train = green[train_cases,2:ncol(green)]
...
ggplot() +
  ...
  xlab("Actual Rent Price") + 
  ylab("Predicted Rent Price") + 
  ggtitle("Actual Rent vs. Predicted Rent for LOGIT1")
...
ggplot() +
  ...
  xlab("Actual Rent Price") + 
  ylab("Predicted Rent Price") + 
  ggtitle("Actual Rent vs. Predicted Rent for LOGIT2")
```

```{r green_logit_rmse, echo=FALSE, comment=NA}
cat("LOGIT1 = LOGISTIC REGRESSION (without interactions) - RMSE:", logm_vals[1])
cat("LOGIT2 = LOGISTIC REGRESSION (with interactions) - RMSE:", logm_vals[2])
```
...
From the plots of each predicted model, we determined that the lasso and ridge prediction models performed the best in predicting rental price of the properties because they produced comparatively lower RMSE values. This is inline with what we expected, as we knew that lasso and ridge were regularlized linear models, which meant they would perform better than a vanilla linear regression model. However, since KNN doesn't have a predetermined model shape and logistic regression is based on a log curve, we had to test out each type of model to determine which would produce the best fit.
...
print(coefficients)
cat("Average change in rental price in regards to green rating ($):", coefficients[13])
```
...
ggplot(data = wine_features) +
  ...
  ggtitle("Pred. Wine Colors from Principal Components") +
  xlab("Color of Wine") +
...
ggplot(data = wine_features) +
  ...
  ggtitle("Pred. Wine Quality from PC Regression") +
  xlab("Quality of Wine") +
  ylab("Pred. Quality of Wine")
...
These plots show a more descriptive explanation of the clusters. From the plot of red vs. white by cluster above, we understood that clusters 1 and 1 are predominantly red wines while clusters 3, 4, and 5 are predominantly white wines. Additionally, the plot of quality by cluster shows that clusters 2, 3, and 5 are roughly mid-quality wines while clusters 1 and 4 contain comparatively higher-quality wines.
...
In this activity, we analyzed data from a market-research study using followers of the Twitter account of a large consumer brand NutrientH20. In order to allow NutrientH20 to hone its messaging a little more sharply, we examined the dataset and highlighted any noteworthy market segments in NutrientH20's social-media audience.

## Data and Model

This social marketing dataset contains counts of Tweets from a random anonyomous sample of the Twitter account's followers over a seven-day period in June 2014. Each Tweet was categorized based on its content using a pre-specified scheme of 36 different categories, each representing a broad area of interest (e.g. politics, sports, family, etc.) and the number of Tweets per anonymous user per category was stored in the social marketing dataset. Our goal in this activity was to identify any notable market segments that we observed in NutrientH20's social-media audience using a variety of dimensionality reduction and clustering techniques.

## PCA

A principal components analysis would be helpful for the dimensionality reduction of this data set because the 36 categories are not useful for interpretation as it is. Since the social marketing dataset contains only numerical, continous data for each category, performing a dimensionality reduction could provide us with better insight into the dataset as a whole.
...
```{r social, include=FALSE, comment=NA}
# Center and scale the data
S = social[,-1]
...
```{r social_pca, include=FALSE, comment=NA}
##### PCA #####
...
```{r social_pca_basic_plots, echo=FALSE, fig.width = 5, fig.height = 3, fig.align='center', comment=NA, warning=FALSE}
## variance plot
plot(PCAsocial)
summary(PCAsocial)
```
...
```{r social_rounded_PCs, echo=FALSE, comment=NA}
# first five PCs
...
ggplot(data = social_features) +
  ...
  ggtitle("Pred. Chatter Tweets from PC Regression") +
  xlab("Chatter Tweets") +
  ylab("Pred. Chatter Tweets")
...
ggplot(data = social_features) +
  ...
  ggtitle("Pred. Cooking Tweets from PC Regression") +
  xlab("Cooking Tweets") +
  ylab("Pred. Cooking Tweets")
...
ggplot(data = social_features) +
  ...
  ggtitle("Pred. Fashion Tweets from PC Regression") +
  xlab("Fashion Tweets") +
  ylab("Pred. Fashion Tweets")
...
ggplot(data = social_features) +
  ...
  ggtitle("Pred. Online Gaming Tweets from PC Regression") +
  xlab("Online Gaming Tweets") +
  ylab("Pred. Online Gaming Tweets")
...
ggplot(data = social_features) +
  ...
  ggtitle("Pred. College/University Tweets from PC Regression") +
  xlab("College/University Tweets") +
  ylab("Pred. College/University Tweets")
...
ggplot(data = social_features) +
  ...
  ggtitle("Pred. Sports Playing Tweets from PC Regression") +
  xlab("Sports Playing Tweets") +
  ylab("Pred. Sports Playing Tweets")
...
```{r social_k, include=FALSE, comment=NA}
##### K-means #####
...
```{r social_k_cluster_size, echo=FALSE, comment=NA}
cat("Size of clusters with K-means clustering:", clust1$size)
cat("Size of clusters with K-means++ clustering:", clust2$size)
```
...
```{r social_kpp_plot_travel, echo=FALSE, fig.width = 5, fig.height = 3, fig.align='center', out.width='.49\\linewidth', fig.show='hold', comment=NA, warning=FALSE}
ggplot(data = social) +
  geom_violin(aes(x = photo_sharing, y = travel, color=factor(clust2$cluster))) +
  ggtitle("Photo Sharing & Travel by Cluster") +
  xlab("Photo Sharing Tweets") +
  ylab("Travel Tweets") +
...
```{r social_kpp_plot_female, echo=FALSE, fig.width = 5, fig.height = 3, fig.align='center', out.width='.49\\linewidth', fig.show='hold', comment=NA, warning=FALSE}
ggplot(data = social) +
  geom_violin(aes(x = beauty, y = fashion, color=factor(clust2$cluster))) +
  ggtitle("Beauty & Fashion by Cluster") +
  xlab("Beauty Tweets") +
  ylab("Fashion Tweets") +
  guides(color=guide_legend(title="Cluster"))
...
ggplot(data = social) +
  ...
  ggtitle("Cooking & Shopping by Cluster") +
  xlab("Cooking Tweets") +
  ylab("Shopping Tweets") +
  guides(color=guide_legend(title="Cluster"))

ggplot(data = social) +
  geom_violin(aes(x = beauty, y = photo_sharing, color=factor(clust2$cluster))) +
  ggtitle("Beauty & Photo Sharing by Cluster") +
  xlab("Beauty Tweets") +
  ylab("Photo Sharing Tweets") +
  guides(color=guide_legend(title="Cluster"))
```
...
```{r social_kpp_plot_sports_food, echo=FALSE, fig.width = 5, fig.height = 3, fig.align='center', out.width='.49\\linewidth', fig.show='hold', comment=NA, warning=FALSE}
ggplot(data = social) +
  geom_violin(aes(x = sports_playing, y = sports_fandom, color=factor(clust2$cluster))) +
  ggtitle("Sports Playing & Sports Fandom by Cluster") +
  xlab("Sports Playing Tweets") +
  ylab("Sports Fandom Tweets") +
  guides(color=guide_legend(title="Cluster"))

ggplot(data = social) +
  geom_violin(aes(x = personal_fitness, y = health_nutrition, color=factor(clust2$cluster))) +
  ggtitle("Personal Fitness & Health Nutrition by Cluster") +
  xlab("Personal Fitness Tweets") +
  ylab("Health Nutrition Tweets") +
  guides(color=guide_legend(title="Cluster"))

ggplot(data = social) +
  geom_violin(aes(x = family, y = food, color=factor(clust2$cluster))) +
  ggtitle("Family & Food by Cluster") +
  xlab("Family Tweets") +
  ylab("Food Tweets") +
  guides(color=guide_legend(title="Cluster"))
```
...
```{r social_kpp_plot_health, echo=FALSE, fig.width = 5, fig.height = 3, fig.align='center', out.width='.49\\linewidth', fig.show='hold', comment=NA, warning=FALSE}
ggplot(data = social) +
  geom_violin(aes(x = online_gaming, y = college_uni, color=factor(clust2$cluster))) +
  ggtitle("Online Gaming & College/University by Cluster") +
  xlab("Online Gaming Tweets") +
  ylab("College/University Tweets") +
  guides(color=guide_legend(title="Cluster"))
```
...
```{r social_hier_complete, include=FALSE, comment=NA}
##### Hierarchical clustering #####
...
```{r social_hier_sizes, echo=FALSE, comment=NA}
# Single linkage
...
cat("Singe linkage cluster sizes:", summary(factor(hier_social_cluster1_single)))
...
cat("Complete linkage cluster sizes:", summary(factor(hier_social_cluster1_complete)))
...
cat("Average linkage cluster sizes:", summary(factor(hier_social_cluster1_average)))
...
cat("Centroid linkage cluster sizes:", summary(factor(hier_social_cluster1_centroid)))
```
...
```{r social_hier_plot_female, echo=FALSE, fig.width = 5, fig.height = 3, fig.align='center', out.width='.49\\linewidth', fig.show='hold', comment=NA, warning=FALSE}
...
ggplot(data = D_complete_social[order(D_complete_social$photo_sharing), ]) + 
  ...
  guides(fill=guide_legend(title="Photo Sharing"))
...
ggplot(data = D_complete_social[order(D_complete_social$cooking), ]) + 
  ...
  guides(fill=guide_legend(title="Cooking"))
...
ggplot(data = D_complete_social[order(D_complete_social$fashion), ]) + 
  ...
  guides(fill=guide_legend(title="Fashion"))
...
ggplot(data = D_complete_social[order(D_complete_social$beauty), ]) + 
  ...
  guides(fill=guide_legend(title="Beauty"))
```
...
```{r social_hier_plot_college, echo=FALSE, fig.width = 5, fig.height = 3, fig.align='center', out.width='.49\\linewidth', fig.show='hold', comment=NA, warning=FALSE}
ggplot(data = D_complete_social[order(D_complete_social$online_gaming), ]) + 
  geom_bar(mapping = aes(x = z, y = 100, fill = online_gaming), position="fill", stat='identity') +
  ggtitle("Proportion of Online Gaming by Cluster") +
  xlab("Cluster") +
  ylab("Proportion of Online Gaming Tweets") +
  guides(fill=guide_legend(title="Online Gaming"))

ggplot(data = D_complete_social[order(D_complete_social$college_uni), ]) + 
  geom_bar(mapping = aes(x = z, y = 100, fill = college_uni), position="fill", stat='identity') +
  ggtitle("Proportion of College/University by Cluster") +
  xlab("Cluster") +
  ylab("Proportion of College/University Tweets") +
  guides(fill=guide_legend(title="College/University"))
```

Another commonality that we found was in cluster 10. These plots show a noticeably high proportion of high numbers of online gaming and college/university Tweets in cluster 10 (the cluster whose bar is a significantly lighter shade). This follows our understanding of the distinct groupings associated with PC4 and PC5 because we had established that these components had a strong association with college and playing video games. Since this grouping involves activites associated with teenagers and young adults, we have solidified our insight into this market segment.
...
```{r social_hier_complete_plots, echo=FALSE, fig.width = 5, fig.height = 3, fig.align='center', out.width='.49\\linewidth', fig.show='hold', comment=NA, warning=FALSE}
#which(hier_social_cluster1_complete == 1)  # Examine the cluster members

D_complete_social = data.frame(social, z = hier_social_cluster1_complete)

# plot proportion of chatter by cluster
ggplot(data = D_complete_social[order(D_complete_social$health_nutrition), ]) + 
  geom_bar(mapping = aes(x = z, y = 100, fill = health_nutrition), position="fill", stat='identity') +
  ggtitle("Proportion of Health/Nutrition Tweets by Cluster") +
  xlab("Cluster") +
  ylab("Proportion of Health/Nutrition Tweets") +
  guides(fill=guide_legend(title="Health/Nutrition"))
...
ggplot(data = D_complete_social[order(D_complete_social$personal_fitness), ]) + 
  ...
  guides(fill=guide_legend(title="Personal Fitness"))
```
...
After conducting a PCA, K-means and K-means++ clustering, and hierarchical clustering with complete linakge, we have determined that there exist a few unique market segments in NutrientH20's social-media audience.
