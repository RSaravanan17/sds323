...
N = nrow(loadhou)  # number of rows in loadhou
N_train = floor(0.8*N)  # store 80% of the number of rows
N_test = N - N_train  # store the remaining number of rows
...
N_train = 150  # sample size of 150
train_ind = sort(sample.int(N, N_train, replace=FALSE))
D_train = loadhou[train_ind,]
D_train = arrange(D_train, KHOU)  # sort by KHOU
y_train = D_train$COAST
X_train = data.frame(KHOU=jitter(D_train$KHOU))

knn_model = knn.reg(X_train, X_train, y_train, k = 3)  # 3 nearest neighbors
...
knn_model = knn.reg(X_train, X_train, y_train, k = 75)  # 75 nearest neighbors
